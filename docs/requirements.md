...
- RLSの実現方式の詳細（`db.TenantTx`、例外関数、DBロール）（§4.3）→ [ADR 0003](adr/0003-rls-implementation.md)
- セッションとパスワードの方式（§5）→ [ADR 0004](adr/0004-sessions-passwords-tokens.md)
- 招待・パスワード再設定トークンの発行方法（§5.1）→ [ADR 0004](adr/0004-sessions-passwords-tokens.md)
- Actorの保存方式（§5.5）→ [ADR 0005](adr/0005-actor-storage.md)
...
M1 の項目（テナント作成用ロールの権限、既存Userの招待の受諾、ユーザー単位の出来事の監査先、ジョブのエラー記録）は、縦断の実装で確かめて [ADR 0003](adr/0003-rls-implementation.md)〜[0007](adr/0007-job-errors-without-personal-data.md) で決めた。
